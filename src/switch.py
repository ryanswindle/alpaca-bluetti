...
async def setasync(devnum: int, params: AlpacaPutParams = Depends(alpaca_put_params)):
    get_device(devnum)
    return MethodResponse.create(
        client_transaction_id=params.client_transaction_id,
...
async def setasyncvalue(devnum: int, params: AlpacaPutParams = Depends(alpaca_put_params)):
    get_device(devnum)
    return MethodResponse.create(
        client_transaction_id=params.client_transaction_id,
...
async def setswitch(devnum: int, Id: Annotated[int, Form()], State: Annotated[str, Form()], params: AlpacaPutParams = Depends(alpaca_put_params)):
    device = get_device(devnum)
    # Validate State value before the connected check so bad input returns
